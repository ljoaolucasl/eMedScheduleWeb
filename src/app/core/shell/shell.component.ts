...
export class ShellComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    if (this.element) {
      this.element.nativeElement
        ...
        .firstChild.setAttribute('d', this.light);
      this.element.nativeElement
  ...
  logout() {
    this.authService.logout().subscribe({
      next: () => this.processSucesso(),
      error: (error: Error) => this.processErro(error),
    });
  }
  ...
  processErro(error: Error) {}

  toggleTheme() {
    this.chosenTheme = !this.chosenTheme;
    this.themeService.toggleTheme(this.chosenTheme);
  }
}
